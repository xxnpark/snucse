...
image[mask] = torch.ones_like(image[mask])
...
recon = image.clone().detach().requires_grad_(True)
optimizer = torch.optim.SGD([recon], lr=1e-3)
...
for i in range(300):
    ...
    recon.data[mask.logical_not()] = image.data[mask.logical_not()]
    recon.data[mask] = recon.data[mask].clamp(min=0, max=1)
