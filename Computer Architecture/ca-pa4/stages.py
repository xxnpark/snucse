...
class BTB(object):
    ...
    def lookup(self, pc):
        ...
        if valid_bit and (lookup_tag == tag):
...
csignals = {
    ...
    PUSH   : [ Y, BR_N  , OP1_RS1, OP2_IMI, OEN_1, OEN_1, ALU_SUB  , WB_X  , REN_0, MEN_1, M_XWR, MT_W, ],
...
class IF(Pipe):
    ...
    def compute(self):
        ...
        self.ex_pcplus4 = Pipe.cpu.adder_pcplus4.op(Pipe.EX.pc, 4)

        target_address, self.btb_status = Pipe.cpu.btb.lookup(self.pc)

        if Pipe.CTL.br_correct:
            target_address = Pipe.EX.brjmp_target
        if Pipe.CTL.br_wrong:
            self.btb_status = False
    ...
    def update(self):
        ...
        if Pipe.CTL.ID_bubble:
            ...
            ID.reg_btb_status   = self.btb_status
...
class ID(Pipe):
    ...
    reg_wb_sp       = False             # ID.reg_wb_sp
    reg_btb_status  = False
    ...
    def compute(self):
        ...
        self.wb_sp      = ID.reg_wb_sp
        self.btb_status = ID.reg_btb_status

        self.rs1        = STACK if RISCV.opcode(self.inst) in [ PUSH, POP ] else \
                          RISCV.rs1(self.inst)          # for CTL (forwarding check)
        ...
        imm_i           = RISCV.sign_extend(4, 12) if RISCV.opcode(self.inst) in [ PUSH, POP ] else \
                          RISCV.imm_i(self.inst)
        ...
        self.op1_data = self.pc         if Pipe.CTL.op1_sel == OP1_PC       else \
                        ...
                        Pipe.MM.alu_out if Pipe.CTL.fwd_op1 == FWD_MM_ALU   else \
                        Pipe.WB.wbdata  if Pipe.CTL.fwd_op1 == FWD_WB       else \
                        Pipe.WB.alu_out if Pipe.CTL.fwd_op1 == FWD_WB_ALU   else \
                        rf_rs1_data
        ...
        self.op2_data = Pipe.EX.alu_out if Pipe.CTL.fwd_op2 == FWD_EX       else \
                        ...
                        Pipe.MM.alu_out if Pipe.CTL.fwd_op2 == FWD_MM_ALU   else \
                        Pipe.WB.wbdata  if Pipe.CTL.fwd_op2 == FWD_WB       else \
                        Pipe.WB.alu_out if Pipe.CTL.fwd_op2 == FWD_MM_ALU   else \
                        alu_op2
        ...
        self.rs2_data = Pipe.EX.alu_out if Pipe.CTL.fwd_rs2 == FWD_EX       else \
                        ...
                        Pipe.MM.alu_out if Pipe.CTL.fwd_rs2 == FWD_MM_ALU   else \
                        Pipe.WB.wbdata  if Pipe.CTL.fwd_rs2 == FWD_WB       else \
                        Pipe.WB.alu_out if Pipe.CTL.fwd_rs2 == FWD_MM_ALU   else \
                        rf_rs2_data
    ...
    def update(self):
        ...
        if Pipe.CTL.EX_bubble:
            ...
            EX.reg_wb_sp            = self.wb_sp
            EX.reg_btb_status       = self.btb_status
...
class EX(Pipe):
    ...
    def compute(self):
        ...
        self.wb_sp              = EX.reg_wb_sp
        self.btb_status         = EX.reg_btb_status
...
class MM(Pipe):
    ...
    def compute(self):
        ...
        self.wb_sp          = MM.reg_wb_sp
        self.op1_data       = MM.reg_op1_data

        # Access data memory (dmem) if needed
        if self.wb_sp and self.c_rf_wen:
            mem_data, status = Pipe.cpu.dmem.access(self.c_dmem_en, self.op1_data, self.rs2_data, self.c_dmem_rw)
        else:
...
class WB(Pipe):
    ...
    def update(self):
        ...
        if self.wb_sp:
            Pipe.cpu.rf.write(STACK, self.alu_out)
...
class Control(object):
    ...
    def gen(self, inst):
        ...
        self.fwd_op2        =   FWD_EX      if (EX.reg_rd == Pipe.ID.rs2) and               \
                                               ...
                                               self.op2_sel == OP2_RS2 or                   \
                                               EX.reg_wb_sp and                             \
                                               (Pipe.ID.rs2 == STACK) and                   \
                                               self.op2_sel == OP2_RS2 else                 \
                                FWD_MM      if (MM.reg_rd == Pipe.ID.rs2) and               \
                                               (MM.reg_rd != 0) and Pipe.MM.c_rf_wen and    \
                                               self.op2_sel == OP2_RS2 or                   \
                                               MM.reg_wb_sp and not Pipe.MM.c_rf_wen and    \
                                               (Pipe.ID.rs2 == STACK) and                   \
                                               self.op2_sel == OP2_RS2 else                 \
                                FWD_MM_ALU  if MM.reg_wb_sp and Pipe.MM.c_rf_wen and        \
                                               (Pipe.ID.rs2 == STACK) and                   \
                                               self.op2_sel == OP2_RS2 else                 \
                                FWD_WB      if (WB.reg_rd == Pipe.ID.rs2) and               \
                                               (WB.reg_rd != 0) and WB.reg_c_rf_wen and     \
                                               self.op2_sel == OP2_RS2 or                   \
                                               WB.reg_wb_sp and not WB.reg_c_rf_wen and     \
                                               (Pipe.ID.rs2 == STACK) and                   \
                                               self.op2_sel == OP2_RS2 else                 \
                                FWD_WB_ALU  if WB.reg_wb_sp and WB.reg_c_rf_wen and         \
                                               (Pipe.ID.rs2 == STACK) and                   \
                                               self.op2_sel == OP2_RS2 else                 \
                                FWD_NONE
        ...
        self.fwd_rs2        =   FWD_EX      if (EX.reg_rd == Pipe.ID.rs2) and rs2_oen and   \
                                               (EX.reg_rd != 0) and EX.reg_c_rf_wen or      \
                                               EX.reg_wb_sp and rs2_oen and                 \
                                               (Pipe.ID.rs2 == STACK) else                  \
                                FWD_MM      if (MM.reg_rd == Pipe.ID.rs2) and rs2_oen and   \
                                               (MM.reg_rd != 0) and Pipe.MM.c_rf_wen or     \
                                               MM.reg_wb_sp and rs2_oen and                 \
                                               not Pipe.MM.c_rf_wen and                     \
                                               (Pipe.ID.rs2 == STACK) else                  \
                                FWD_MM_ALU  if MM.reg_wb_sp and rs2_oen and                 \
                                               Pipe.MM.c_rf_wen and                         \
                                               (Pipe.ID.rs2 == STACK) else                  \
                                FWD_WB      if (WB.reg_rd == Pipe.ID.rs2) and rs2_oen and   \
                                               (WB.reg_rd != 0) and WB.reg_c_rf_wen or      \
                                               WB.reg_wb_sp and rs2_oen and                 \
                                               not WB.reg_c_rf_wen and                      \
                                               (Pipe.ID.rs2 == STACK) else                  \
                                FWD_WB_ALU  if WB.reg_wb_sp and rs2_oen and                 \
                                               WB.reg_c_rf_wen and                          \
                                               (Pipe.ID.rs2 == STACK) else                  \
                                FWD_NONE
